...
const SEED_COMPANIES = ['字节跳动', '阿里', '美团', '腾讯', '华为'];
...
export class SeedService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    if (companyCount === 0) {
      const fromSeed = [
        ...SEED_COMPANIES,
        ...SEED_QUESTIONS.flatMap((q) => q.companies ?? []),
      ];
      const fromQuestions = await this.questionModel.distinct('companies');
      const fromEvents = await this.eventModel.distinct('company');
      const names = [...new Set([...fromSeed, ...fromQuestions, ...fromEvents])]
